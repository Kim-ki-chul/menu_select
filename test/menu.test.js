// menu.js의 후보 메뉴 풀 구성 로직에 대한 최소 self-check
...
// count를 풀 크기만큼 주면 풀 전체가 그대로 나와야 한다 (최근 추천 기록으로 걸러내지 않음)
const lunchPool = CATEGORY_TABLES['점심']['보통'];
const lunchCandidates = getCandidateMenus('점심', '보통', '아무거나', lunchPool.length);
assert.strictEqual(lunchCandidates.length, lunchPool.length, '풀 크기만큼 요청했는데 후보가 줄어듦');
