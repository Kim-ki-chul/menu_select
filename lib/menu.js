// 카테고리(아침/점심/저녁/회식) x 날씨 버킷별 후보 메뉴 테이블과 "최근에 고른 것과 겹치지 않게" 제외 로직
...
function getCandidateMenus(category, bucket, cuisine, history, count = 3) {
  const table = CATEGORY_TABLES[category] || CATEGORY_TABLES['점심'];
  const pool = table[bucket] || table['보통'];
  ...
  // 최근 3개와 안 겹치게 → 그래도 다 빠지면 직전 1개만 피하기 → 그래도 없으면 전체 풀
  const recent3 = getRecentMenus(history, category, 3);
  const excludingRecent3 = basePool.filter((m) => !recent3.includes(m));
  const lastMenu = getLastMenu(history, category);
  const excludingLast1 = basePool.filter((m) => m !== lastMenu);
  const usable = excludingRecent3.length > 0 ? excludingRecent3 : excludingLast1.length > 0 ? excludingLast1 : basePool;

  return pickRandom(usable, Math.min(count, usable.length));
}
